...
public:
    ...
    // check if a set of keys are available for given qty (simple version)
    ...
    // new: check usage vector <ingredientKey, perSandwichQty> for requested count
    bool hasEnoughUsage(const vector<pair<string,int>> &usagePerSandwich, int requestedCount) {
        for (const auto &p : usagePerSandwich) {
            const string &key = p.first;
            int per = p.second;
            long long needed = (long long)per * requestedCount;
            if (stock.find(key) == stock.end()) return false;
            if (stock[key] < needed) return false;
        }
        return true;
    }

    // consume single key qty
    void consume(const string &key, int qty) {
        if (stock.find(key) != stock.end()) stock[key] -= qty;
    }

    // consume list of keys by qty (simple)
    void consumeAll(const vector<string> &keys, int qty) {
        for (auto &k : keys) consume(k, qty);
    }

    // new: consume per-sandwich usage multiplied by count
    void consumeUsage(const vector<pair<string,int>> &usagePerSandwich, int count) {
        for (const auto &p : usagePerSandwich) {
            const string &k = p.first;
            int per = p.second;
            if (stock.find(k) != stock.end()) stock[k] -= per * count;
        }
    }

    // restore single key qty
    void restore(const string &key, int qty) {
        if (stock.find(key) != stock.end()) stock[key] += qty;
    }

    // restore usages list multiplied by count
    void restoreUsage(const vector<pair<string,int>> &usagePerSandwich, int count) {
        for (const auto &p : usagePerSandwich) {
            const string &k = p.first;
            int per = p.second;
            if (stock.find(k) != stock.end()) stock[k] += per * count;
...
public:
    ...
    double price;
    // per-sandwich usage: pairs <ingredientKey, qtyPerSandwich>
    vector<pair<string,int>> chosenIngredientUsage;

    CustomSandwich() : price(0) {}

    // Build sandwich — does NOT consume stock yet (consumption happens when user confirms quantity)
    Item buildSandwich() {
        chosenIngredientUsage.clear();
        price = 0;
        ...
        if (b == 1) {
            ...
            chosenIngredientUsage.push_back({breadKey, 1}); // 1 bread
        } else if (b == 2) {
            bread = "Brown"; breadKey = "Bread_Brown"; price += 60;
            chosenIngredientUsage.push_back({breadKey, 1});
        } else {
        ...
        if (p == 1) { protein = "Chicken"; proteinKey = "Protein_Chicken"; price += 150; chosenIngredientUsage.push_back({proteinKey, 2}); } // chicken uses 2 portions
        else if (p == 2) { protein = "Beef"; proteinKey = "Protein_Beef"; price += 200; chosenIngredientUsage.push_back({proteinKey, 2}); } // beef uses 2 portions
        else if (p == 3) { protein = "Egg"; proteinKey = "Protein_Egg"; price += 70; chosenIngredientUsage.push_back({proteinKey, 1}); } // egg uses 1
        ...
        if (v == 1) { veggies = "Lettuce"; vegKey = "Veg_Lettuce"; price += 40; chosenIngredientUsage.push_back({vegKey, 1}); }
        else if (v == 2) { veggies = "Cucumber"; vegKey = "Veg_Cucumber"; price += 50; chosenIngredientUsage.push_back({vegKey, 1});}
        else if (v == 3) { veggies = "Olives"; vegKey = "Veg_Olives"; price += 30; chosenIngredientUsage.push_back({vegKey, 1});}
        else if (v == 4) { veggies = "Onions"; vegKey = "Veg_Onions"; price += 20; chosenIngredientUsage.push_back({vegKey, 1});}
        ...
        if (s == 1) { sauce = "Mayo"; sauceKey = "Sauce_Mayo"; price += 20; chosenIngredientUsage.push_back({sauceKey, 1}); }
        else if (s == 2) { sauce = "Garlic"; sauceKey = "Sauce_Garlic"; price += 20; chosenIngredientUsage.push_back({sauceKey, 1}); }
        else if (s == 3) { sauce = "Chipotle"; sauceKey = "Sauce_Chipotle"; price += 20; chosenIngredientUsage.push_back({sauceKey, 1}); }
        ...
        if (e == 1) { extras = "Cheese"; extraKey = "Extra_Cheese"; price += 50; chosenIngredientUsage.push_back({extraKey, 2}); } // extra cheese uses 2 portions
        else { extras = "None"; extraKey = ""; }

        string name = "Custom Sandwich (" + bread + ", " + protein + ", " + veggies + ", " + sauce + ", " + extras + ")";
        // quantity field not used for custom sample, usage stored in chosenIngredientUsage
...
public:
    ...
    Menu() {
        ...
        savoury.addItem(Item("Make Your Own Sandwich", 100, 5)); // option
...
public:
    ...
    int quantity;
    bool isCustom;
    // per-sandwich ingredient usage (ingredientKey, qtyPerSandwich)
...
public:
    ...
    void addItem(const string &name, double price, int qty, bool isCustom=false, const vector<pair<string,int>> &usage = {}) {
        ...
        for (auto &ci : items) {
            ...
            // For custom sandwiches, treat same composition as same item
    ...
    void displayCart() {
        ...
        for (size_t i = 0; i < items.size(); i++) {
            ...
            if (items[i].isCustom) {
                cout << "   Ingredients used per sandwich:\n";
                for (auto &p : items[i].ingredientUsage)
                    cout << "    - " << p.first << " (x" << p.second << " per sandwich)\n";
            }
        }
                ...
                if (cat > 0) {
                    ...
                    if (itemNum > 0) {
                        ...
                        if (selected->name == "Make Your Own Sandwich") {
                            CustomSandwich cs;
                            Item customSample = cs.buildSandwich(); // chosenIngredientUsage stored in cs
                            ...
                            // check ingredient availability using per-sandwich usage
                            if (!ingredientInventory.hasEnoughUsage(cs.chosenIngredientUsage, qty)) {
                                cout << "❌ Not enough ingredient stock for " << qty << " sandwich(es).\n";
                                // show available quantities for the missing ones
                                for (auto &p : cs.chosenIngredientUsage) {
                                    cout << "   " << p.first << ": " << ingredientInventory.stock[p.first] << " available (needs " << p.second * qty << ")\n";
                                }
                            } else {
                                // consume ingredients in correct multiples
                                ingredientInventory.consumeUsage(cs.chosenIngredientUsage, qty);
                                // add to cart as custom; store per-sandwich usage (not multiplied)
                                student.cart.addItem(customSample.name, customSample.price, qty, true, cs.chosenIngredientUsage);
                            }
                        } else {
                ...
                if (!student.cart.isEmpty()) {
                    ...
                    if (removeIndex > 0) {
                        ...
                        if (itemToRemove.isCustom) {
                            ...
                            ingredientInventory.restoreUsage(itemToRemove.ingredientUsage, qtyToRemove);
                        } else {
